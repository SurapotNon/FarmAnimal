# 4 fitting the Linear Mixed-Effects Model (LMM)
# Load necessary libraries
library(lme4)
library(ggplot2)

# Fit the initial LMM model
...
for (iteration in 1:iterations) {
  ...
  for (i in seq_along(clustering_vars)) {
    ...
    # Fit the LMM Model, excluding the clustering variable from fixed effects
    fixed_effects <- setdiff(c("AgeMonths", "Sex", "Sire", "type", "BirthType", "DW", "DurationBeforeSlaughter", "MotherAgeOnBornDate"), clustering_var)
    formula <- as.formula(paste("Weight ~", paste(fixed_effects, collapse = " + "), "+ (1 + AgeMonths |", clustering_var, ")"))
    ...
    predictions <- tryCatch({
      predict(lmm_model, newdata = test_data, allow.new.levels = TRUE)
...
# Fit the final LMM model using the best clustering variable, excluding it from fixed effects
fixed_effects <- setdiff(c("AgeMonths", "Sex", "Sire", "type", "BirthType", "DW", "DurationBeforeSlaughter", "MotherAgeOnBornDate"), best_clustering_var)
final_formula <- as.formula(paste("Weight ~", paste(fixed_effects, collapse = " + "), "+ (1 + AgeMonths |", best_clustering_var, ")"))

final_lmm_model <- lmer(final_formula, data = combined_lamb_removemissing)

# Summary of the final model
summary(final_lmm_model)

# Compute 95% confidence intervals for the model coefficients
conf_intervals <- confint(final_lmm_model, level = 0.95)
...
df <- n - length(fixef(final_lmm_model))

# Critical t value for 95% confidence interval
t_critical <- qt(0.975, df)

# Extract summary table of the model
summary_table <- summary(final_lmm_model)$coefficients
...
fitted_values <- fitted(final_lmm_model)
residuals <- resid(final_lmm_model)
