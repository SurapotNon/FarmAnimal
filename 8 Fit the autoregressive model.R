# 8. Fit the autoregressive model
library(nlme)
...
fit_ar2 <- lme(
  Weight ~ AgeMonths + Sire + type + BirthType + DW + DurationBeforeSlaughter + MotherAgeOnBornDate, 
  random = ~ 1 | No., 
  correlation = corARMA(form = ~ AgeMonths | No., p = 1, q = 1), 
  data = combined_lamb_removemissing
)
...
for (iteration in 1:iterations) {
  ...
  performance_metrics <- rep(NA, 3)
  
  # Define models with different correlation structures
  models <- list(
    list(
      name = "corAR1",
      fit = tryCatch({
  ...
  for (i in seq_along(models)) {
    ...
    predictions <- tryCatch({
      predict(model, newdata = test_data, allow.new.levels = TRUE)
...
model_names <- sapply(models, function(m) m$name)
...
best_model_index <- as.integer(names(which.max(winning_counts)))
best_model_name <- model_names[best_model_index]

# Fit the final model using the best correlation structure
best_model_fit <- models[[best_model_index]]$fit

# Summary of the final model
summary(best_model_fit)

# Compute 95% confidence intervals for the model coefficients
conf_intervals <- confint(best_model_fit, level = 0.95)
...
df <- n - length(fixef(best_model_fit))

# Critical t value for 95% confidence interval
t_critical <- qt(0.975, df)

# Extract summary table of the model
summary_table <- summary(best_model_fit)$tTable

# Compute confidence intervals for each coefficient
conf_intervals <- data.frame(
  Estimate = summary_table[, "Value"],
  Std_Error = summary_table[, "Std.Error"],
  Lower_95 = summary_table[, "Value"] - t_critical * summary_table[, "Std.Error"],
  Upper_95 = summary_table[, "Value"] + t_critical * summary_table[, "Std.Error"]
)
...
residuals <- resid(best_model_fit)
